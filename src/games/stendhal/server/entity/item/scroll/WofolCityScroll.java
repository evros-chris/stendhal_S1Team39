...
/**
 * Represents a teleport scroll to wofol city.
 */
public class WofolCityScroll extends MarkedScroll {
	...
	/**
	 * Creates a new Wofol city teleport scroll.
	 *
	 * @param name
	 * @param clazz
	 * @param subclass
	 * @param attributes
	 */
	public WofolCityScroll(final String name, final String clazz, final String subclass,
	...
	protected boolean useTeleportScroll(final Player player) {
		// init as home_scroll
		StendhalRPZone zone = SingletonRepository.getRPWorld().getZone("-1_semos_mine_nw");
		int x = 52;
		int y = 68;

		/*
		 * Wofol city scrolls have a destination which is stored in the infostring,
		...
		if (infostring != null) {
			...
			if (st.countTokens() == 3) {
				...
				if (temp == null) {
					...
					player.sendPrivateText("Oh oh. For some strange reason the scroll did not teleport me to the right place. Upon it is written: -1_semos_mine_nw");
					logger.warn("marked scroll to unknown zone " + infostring
							+ " teleported " + player.getName()
							+ " to Wofol city instead");
				} else {
